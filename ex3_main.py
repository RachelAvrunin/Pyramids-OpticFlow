...
def blendDemo():
    im1 = cv2.cvtColor(cv2.imread('Kevin-Hart.jpg'), cv2.COLOR_BGR2RGB) / 255
    im2 = cv2.cvtColor(cv2.imread('babyCry.jpg'), cv2.COLOR_BGR2RGB) / 255
    mask = cv2.cvtColor(cv2.imread('face_mask.jpg'), cv2.COLOR_BGR2RGB) / 255
    ...
    cv2.imwrite('baby Kevin.png', cv2.cvtColor((im_blend * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))


def main():
    img_path = 'hairMovment.png'
    lkDemo(img_path)
    img_path = 'walkInTheRain.jpg'
